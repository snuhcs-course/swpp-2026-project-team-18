...
data class RouteArrival(
    ...
    val crowding: String = "",
    /**
     * 지하철 "급행" / "특급". 일반이거나 모르면 빈 문자열.
     *
     * 이것으로 열차를 목록에서 **지우지 않는다.** 급행이 사용자의 하차역을
     * 지나치는지는 서울 열린데이터 응답에 정차 패턴이 없어 알 수 없다. 숨기면
     * 더 빠른 선택지를 말없이 빼앗고, 지나친다고 단정하면 거짓이 된다.
     */
    val trainKind: String = "",
    /** 막차인가. 놓치면 집에 못 가므로 [crowding] 보다 강하게 표시한다. */
