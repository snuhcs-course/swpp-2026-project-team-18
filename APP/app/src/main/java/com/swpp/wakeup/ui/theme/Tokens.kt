...
object JitColor {
     ...
     * 더 빠른 길. 지도에만 쓴다.
     *
     * 처음에는 [Green] 으로 그렸는데 같은 화면의 진행 바가 초록을 **정시 도착**
     * 으로 쓰고 있어서, 한 색이 두 뜻이 됐다. 초록은 정시에 남겨 두고 이 색으로
     * 옮겼다.
     *
     * [Blue] 와 충분히 떨어져 있어야 한다 — 고른 경로의 지나온 구간이 파랑이고,
     * 두 선이 나란히 놓인다. 그래서 보라 중에서도 자홍 쪽인 purple-400 을
     * 골랐다. 나머지 토큰과 같은 -400 계열이라 밝은 지도 위에서도 파랑과 같은
     * 정도로 읽힌다.
     */
