...
data class AlarmPlanView(
    ...
    /**
     * 지금 더 빠른 대안 경로의 폴리라인. 지도에 보라 실선으로 겹쳐 그린다.
     *
     * **출발지 기준이다.** 이미 출발한 뒤에는 쓰지 않는다 — 그때는 현재 위치부터
     * 다시 조회한 [LiveRoute] 가 이 자리를 덮는다.
     *
     ...
     * 그리지 않는다. 선만 덩그러니 있으면 사용자는 그것이 무엇인지 알 수 없고,
     * 기준점이 출발지인지 현재 위치인지도 구별할 수 없다.
     */
