...
- `pytest` 585건 — 메모리 SQLite. pytest 는 테스트 데이터베이스를 **만들고
...
```bash
cd backend && python -m pytest                       # 585
cd backend && python scripts/run_local_suite.py      # 8개 스크립트, 서버 기동까지 알아서 한다
cd APP && ./gradlew testDebugUnitTest lintDebug assembleDebug  # 301 + lint
```
...
| 잡 | 하는 일 |
| --- | --- |
| `백엔드 pytest` | `config.settings.test` 로 585건. 메모리 SQLite 고정이라 환경변수를 하나도 주지 않는다 |
